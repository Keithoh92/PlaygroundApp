            ...
            effect.collect { effect ->
                when (effect) {
                    ...
                    is ExpensesActivityEffect.FetchFromThisActivity -> {
                        // Fetching Data from Activity using interactor
                        interactor.fetchDataFromActivity(
                            ExpensesActivityInteractorEvents.FetchDataFromActivity(
                                (effect.onDataFetchedFromActivity)
                            )
                        )

                        // Fetching data from activity using callback
                        fetchDataFromActivity { (effect.onDataFetchedFromActivity) }
                    }
                }
            }
        }
